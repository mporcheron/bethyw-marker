...
class coursework:
  ...
  def retest_autograder(student_id, marks, feedback):
    ...
    try:
      ...
      if (autograder_marks[test[catch2][extended]["10"]] + autograder_marks[test[catch2][extended]["10a"]] < 2) or \
         (autograder_marks[test[output][extended]["02"]] + autograder_marks[test[output][extended]["02a"]] < 1) or \
      ...
      return stage_result(
        updated_label = "No need to retest - " + str(autograder_marks[test[catch2][extended]["10"]] + autograder_marks[test[catch2][extended]["10a"]]) + " / " +
                                                 str(autograder_marks[test[output][extended]["02"]] + autograder_marks[test[output][extended]["02a"]]) + " / " +
                                                 str(autograder_marks[test[output][extended]["05"]] + autograder_marks[test[output][extended]["05a"]]) + " / " +
                                                 str(autograder_marks[test[output][extended]["06"]] + autograder_marks[test[output][extended]["06a"]]),
        next_stage    = "autograder")
  ...
  def autograder(student_id, marks, feedback):
    ...
    try:
      ...
      if autograder_marks[test[catch2][extended]["10"]] > 0 or autograder_marks[test[catch2][extended]["10a"]] > 0:
        ...
        multidecision = [
            {
             ...
             "out1":      ("Output alignment not quite right",        -1, preamble + "2.1. FORMATTED 'TABLES' OUTPUT\nYour output for the tables formatting is neat although the alignment doesn't seem quite right. I looked at the 'popden' dataset here and compared it with what I expected. "),
  ...
  def extra_features(student_id, marks, feedback): # max +3
    multidecision = [
        ...
        {
         "regex1":    ("Didn't use regex",                                                                     0, ""),
         "regex0":    ("Used regex",                                                                           1, "Also, using regular expressions (regex) in your code was a nice and advanced approach. "),
        },
