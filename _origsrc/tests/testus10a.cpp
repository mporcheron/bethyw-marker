...
SCENARIO( "complete-popu1009-popden.csv can be correctly parsed", "[Areas][complete-popu1009-popden-alt]" ) {
  ...
  GIVEN( "a newly constructed Areas instance" ) {
    ...
    AND_GIVEN( "a valid complete-popu1009-popden.csv file as an open std::istream" ) {
      ...
      THEN( "the Areas instance will be populated without exception" ) {
        ...
        REQUIRE_NOTHROW( areas.populateFromAuthorityByYearCSV(stream, BethYw::InputFiles::DATASETS[4].COLS, &areasFilter, &measuresFilter, &yearsFilter) );
